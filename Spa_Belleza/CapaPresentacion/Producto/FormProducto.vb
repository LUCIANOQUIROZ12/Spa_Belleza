...
Public Class FormProducto
    Private ReadOnly idProducto As Integer = 1 ' ID de ejemplo
    ...
    Private Sub BtnRegistrar_Click(sender As Object, e As EventArgs) Handles BtnRegistrar.Click
        ' Crear y mostrar el formulario de registro de nuevo producto
        Dim formRegistrar As New FormRegistrarProducto()
        formRegistrar.ShowDialog()

        ' Recargar los datos en el DataGridView después de registrar un nuevo producto
        CargarProductosEnDataGridView()
    End Sub
    ...
    Private Sub txtBusqueda_Enter(sender As Object, e As EventArgs) Handles txtBusqueda.Enter
        If txtBusqueda.Text = "Nombre Producto" Then
            txtBusqueda.Text = ""
    ...
    Private Sub txtBusqueda_Leave(sender As Object, e As EventArgs) Handles txtBusqueda.Leave
        ...
        FiltrarProductos(txtBusqueda.Text)
    End Sub
    ...
    ' Método para filtrar los productos según la letra con la que empieza el nombre
    Private Sub FiltrarProductos(busqueda As String)
        Dim productosData As DataTable = CType(DataGridView1.DataSource, DataTable)
        If productosData IsNot Nothing Then
            If Not String.IsNullOrWhiteSpace(busqueda) AndAlso busqueda <> "Nombre Producto" Then
                ' Aplicar el filtro para que muestre solo los nombres que empiezan con la letra ingresada
                Dim filtro As String = $"NombreProducto LIKE '{busqueda}%'"
                productosData.DefaultView.RowFilter = filtro
            Else
                ' Limpiar el filtro si no hay texto en el cuadro de búsqueda
                productosData.DefaultView.RowFilter = String.Empty
            End If
        End If
    End Sub
    ...
    Private Sub DataGridView1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        If e.RowIndex >= 0 Then
            ...
            If e.ColumnIndex >= 0 AndAlso e.ColumnIndex < DataGridView1.Columns.Count Then
                ...
                If DataGridView1.Rows(e.RowIndex).Cells("ProductoID") IsNot Nothing AndAlso
                   DataGridView1.Rows(e.RowIndex).Cells("ProductoID").Value IsNot Nothing AndAlso
                   Integer.TryParse(DataGridView1.Rows(e.RowIndex).Cells("ProductoID").Value.ToString(), productoID) Then

                    Select Case columna
                        Case "Editar"
                            ' Llamar al formulario de registro con el ID del producto
                            Dim formRegistrar As New FormRegistrarProducto(productoID)
                            formRegistrar.ShowDialog()

                            ' Recargar los datos en el DataGridView después de editar un producto
                            CargarProductosEnDataGridView()

                        Case "Eliminar"
                            ' Confirmar eliminación
                            Dim resultado As DialogResult = MessageBox.Show("¿Está seguro de que desea eliminar este producto?", "Confirmar Eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
                            If resultado = DialogResult.Yes Then
                                ' Eliminar el producto
                                productoNegocio.EliminarProducto(productoID)

                                ' Recargar los datos en el DataGridView después de eliminar un producto
                                CargarProductosEnDataGridView()
                            End If
                    End Select
                Else
                    MessageBox.Show("El ID del producto no está disponible o es inválido.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
            Else
    ...
    Private Sub BtnUsuarios_Click(sender As Object, e As EventArgs)
        ...
        ' Crear una instancia del formulario de usuarios
        Dim formUsuarios As New FormUsuarios()

        ' Configura el formulario de usuarios para que se ajuste al panel
        formUsuarios.TopLevel = False
        formUsuarios.FormBorderStyle = FormBorderStyle.None
        formUsuarios.Dock = DockStyle.Fill

        ' Agregar el formulario de usuarios al panel y mostrarlo
